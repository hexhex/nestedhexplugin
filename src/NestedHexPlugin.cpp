...
{
public:
	NestedHexPlugin::CtxData& ctxdata;

	NestedHexParserModuleSemantics(ProgramCtx& ctx):
		HexGrammarSemantics(ctx),
		ctxdata(ctx.getPluginData<NestedHexPlugin>())
	{
	}
...
struct sem<NestedHexParserModuleSemantics::nhexAtom>
{
	...
	ID& target)
	{
		...
		NestedHexPlugin::CtxData& ctxdata = mgr.ctx.getPluginData<NestedHexPlugin>();
...
{
	return reinterpret_cast<void*>(& dlvhex::theNestedHexPlugin);
}
